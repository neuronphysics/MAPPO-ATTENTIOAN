...
class SeparatedReplayBuffer(object):
    def __init__(self, args, obs_space, share_obs_space, act_space):
        ...
        self.next_share_obs = np.zeros((self.episode_length + 1, self.n_rollout_threads, *share_obs_shape),
        ...
        self.skills = np.zeros((self.episode_length, self.n_rollout_threads, self._skill_dim),
                               dtype=np.float32)  # Assuming skill_dim is defined
    ...
    def chooseinsert(self, share_obs, obs, next_share_obs, next_obs, rnn_states, rnn_states_critic, actions,
                     action_log_probs,
                     value_preds, rewards, masks, intrinsic_rewards, skills, bad_masks=None, active_masks=None,
                     available_actions=None):
        ...
        if self._use_proper_time_limits:
                ...
                for step in reversed(range(self.rewards.shape[0])):
                    if self._use_popart or self._use_valuenorm:
                        delta = (self.rewards[step] + self._alpha * self.intrinsic_rewards[
                            step]) + self.gamma * value_normalizer.denormalize(self.value_preds[
                                                                                   step + 1]) * self.masks[
                        ...
                        delta = (self.rewards[step] + self._alpha * self.intrinsic_rewards[step]) \
                                + self.gamma * self.value_preds[step + 1] * self.masks[step + 1] - self.value_preds[
                                    step]
                        gae = delta + self.gamma * self.gae_lambda * self.masks[step + 1] * gae
                        gae = gae * self.bad_masks[step + 1]
                ...
                for step in reversed(range(self.rewards.shape[0])):
                    if self._use_popart:
                        self.returns[step] = (self.returns[step + 1] * self.gamma * self.masks[step + 1]
                                              + (self.rewards[step] + self._alpha * self.intrinsic_rewards[step])) \
            ...
            if self._use_gae:
                ...
                for step in reversed(range(self.rewards.shape[0])):
                    if self._use_popart or self._use_valuenorm:
                        delta = (self.rewards[step] + self._alpha * self.intrinsic_rewards[step]) + \
                                self.gamma * value_normalizer.denormalize(self.value_preds[step + 1]) * self.masks[
                                    step + 1] - value_normalizer.denormalize(self.value_preds[step])
                        gae = delta + self.gamma * self.gae_lambda * self.masks[step + 1] * gae
                        self.returns[step] = gae + value_normalizer.denormalize(self.value_preds[step])
                    else:
                        delta = (self.rewards[step] + self._alpha * self.intrinsic_rewards[step]) + self.gamma * \
                                self.value_preds[step + 1] * self.masks[step + 1] - self.value_preds[step]
                        gae = delta + self.gamma * self.gae_lambda * self.masks[step + 1] * gae
                        self.returns[step] = gae + self.value_preds[step]
            else:
                self.returns[-1] = next_value
    ...
    def naive_recurrent_generator(self, advantages, num_mini_batch):
        ...
        for start_ind in range(0, n_rollout_threads, num_envs_per_batch):
            ...
            print(
                f"inside separated_buffer.py, rnn_states_batch.shape is {rnn_states_batch.shape} and rnn_states_critic_batch.shape is {rnn_states_critic_batch.shape}")
